...
// Hand-editable ROOT macro for the global Lambda polarization plot.
// Units:
//   x = sqrt(s_NN) in GeV
//   y/stat/sys = P_Lambda in percent
//
// Run from BESII_Review_PolCorr:
//   root -l -b -q macros/plotLambdaGlobalPolarization_manual.C

namespace LambdaPolManual {
...
const double kYMax = 7.8;
const char *kOutputBase = "figures/lambda_global_polarization_vs_energy_scaled_alice";
...
void plotLambdaGlobalPolarization_manual()
{
  using namespace LambdaPolManual;

  style();

  TCanvas *c = new TCanvas("cLambdaGlobalPolarizationManual", "", 900, 700);
...
//    drawText(2.1, 7.35, "Global #Lambda Polarization", 42, 0.045);
  ...
  frame->GetYaxis()->SetTitle("Global P_{#Lambda} (%)");
  frame->GetYaxis()->SetTitleOffset(0.90);
  frame->Draw();

  drawLine(kXMin, 0.0, kXMax, 0.0, 2, 2, kGray + 2);

  TLegend *legTheory = new TLegend(0.71, 0.50, 0.99, 0.92);
  legTheory->SetFillStyle(4000);
  legTheory->SetBorderSize(0);
  legTheory->SetTextSize(0.030);

  TLegend *legData = new TLegend(0.47, 0.68, 0.70, 0.90);
  legData->SetFillStyle(4000);
  legData->SetBorderSize(0);
