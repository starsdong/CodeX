...
std::vector<PredictionInput> readManifest(const std::string &manifestPath) {
  ...
  while (std::getline(fin, line)) {
    ...
    std::getline(ss, field, ',');
    row.muSMeV = std::stod(field);
    std::getline(ss, field, ',');
    row.muQMeV = std::stod(field);
    std::getline(ss, field, ',');
...
int main(int argc, char *argv[]) {
  ...
  fout << "energy_GeV,T_MeV,muB_MeV,muS_MeV,muQ_MeV,gammaS,R_fm,dVdy_fm3,"
          "prediction_file,ensemble,interpolation_note\n";
  ...
  for (const auto &entry : energies) {
    ...
    par.FixMuS(entry.muSMeV / 1000.0, 0.0);
    par.FixMuQ(entry.muQMeV / 1000.0, 0.0);
    ...
    TTMThermalFitBSQ fit(&set, &par, entry.predictionFile.c_str());
    ...
    fout << std::fixed << std::setprecision(6)
         ...
         << entry.tMeV << ","
         << entry.muBMeV << ","
         << pMuS->GetValue() * 1000.0 << ","
         << pMuQ->GetValue() * 1000.0 << ","
         << entry.gammaS << ","
         << entry.rFm << ","
         << dVdy << ","
         << entry.predictionFile << ",GCanonical,"
         << "\"" << entry.interpolationNote << "\"\n";
    ...
    for (const auto &p : points) {
      ...
      if (!y) {
        y = fit.GetYield(p.pdg, 0, "PREDICTION");
      }
  ...
  std::cout << "Wrote GCE prediction summary: " << summaryPath << "\n";
  std::cout << "Wrote GCE prediction points: " << pointsOutDir << "\n";
  return 0;
}
