...
def load_measured_rows() -> dict[tuple[float, str], dict[str, str]]:
    ...
    # Supplement missing high-energy strange-hadron points from the merged table that
    # already carries 62.4 and 200 GeV measurements from THERMUS fit-point CSVs.
    with STRANGE_MERGED_CSV.open(encoding="utf-8") as f:
        for row in csv.DictReader(f):
            particle = row["particle"]
            if particle not in PARTICLE_ORDER:
                continue
            if not row.get("data_yield"):
                continue
            key = (float(row["energy_GeV"]), particle)
            if key in merged:
                continue
...
def attach_model_rows(merged: dict[tuple[float, str], dict[str, str]], model_dir: Path, column: str) -> None:
...
def main() -> None:
    ...
    attach_model_rows(merged, GC_LOW_DIR, "gc_effective_yield")
    attach_model_rows(merged, GC_FIT_DIR, "gc_effective_yield")
    attach_model_rows(merged, SCE_BLEND_DIR, "sce_blended_yield")
    ...
    model_handles = [
        ...
        Line2D(
            ...
            label="THERMUS GC (effective trace)"
        ),
        Line2D(
            [0], [0], color="black", lw=1.7, ls="-.",
