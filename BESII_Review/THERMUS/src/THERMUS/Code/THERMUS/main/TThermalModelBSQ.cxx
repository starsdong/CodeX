...
TTMThermalModelBSQ::TTMThermalModelBSQ()
{
  ...
  fStrange = fBaryon = fCharge = fDensity = fWroblewski = fCharm = 0.;
...
{
  // Calculates the Primordial particle densities and populates the density 
  // hash table after first constraining muS and/or muQ if required. The 
  ...
  if (fParm->GetMuSConstrain() && fParm->GetMuQConstrain()) {
    check = BSQConstrainSQ(this);
    if (check) {
      cout << "Not Constrained" << endl;
    }
  } else if (fParm->GetMuSConstrain()) {
    check = BSQConstrainS(this);     
    if (check) {
      cout << "Not Constrained" << endl;
    }
  } else if (fParm->GetMuQConstrain()) {
...
Int_t TTMThermalModelBSQ::ConstrainEoverN(Double_t EoverN)
{
  ...
  // muQ or just muS are also to be constrained. The other possibilities 
...
Int_t TTMThermalModelBSQ::ConstrainPercolation()
{
  ...
  if(fParm->GetMuQConstrain() && fParm->GetMuSConstrain()){
                                                           ...
                                                           TTMParameterSetBSQ* CopyParm, Double_t P)
{
  ...
  Double_t mu = CopyPart->GetB()*fParm->GetMuB() + CopyPart->GetS()*fParm->GetMuS() + CopyPart->GetQ()*fParm->GetMuQ() + CopyPart->GetCharm()*fParm->GetMuC();
...
Int_t TTMThermalModelBSQ::ConstrainTotalBaryonDensity(Double_t nb)
{
  ...
  // both muS and muQ are also to be constrained. The other possibilities are 
