...
//__________________________________________________________________________
TString TTMThermalFit::GetName(Int_t id1,Int_t id2,TString descr)
{
  TString name;
...
{
  TString name = GetName(id1,id2,descr);
...
void TTMThermalFit::InputExpYields(char *file)
{
  ...
  if (!data) {
    ...
    Int_t flags[50];          // records whether yield (0) or ratio (1)
    ...
    for (Int_t i = 1; i <= data_points; i++) {
      ...
      if (flags[i - 1] == 0) {	//Yield
        measure >> ID_1;
        measure >> descr;
        measure >> value;
        measure >> error;

        name = GetName(ID_1,0,descr);
...
void TTMThermalFit::GenerateYields()
{
  ...
  while ((next_yield = (TTMYield *) next())) {
    Int_t id_1, id_2;
