...
TTMThermalModelBSQ *gModelBSQConSBDens;
Double_t gBSQySBDens[2];

Int_t BSQConstrainSBDens(TTMThermalModelBSQ *model, Double_t nb)
{    

  gModelBSQConSBDens = model;
  ...
  gBSQySBDens[0] = model->GetParameterSet()->GetSDens();
  gBSQySBDens[1] = nb;
  ...
  if(gBSQySBDens[1] == 0.){
    ...
    broydn(x,2,&check,BSQfuncSBDens);
    BSQfuncSBDens(2,x,fbroydn);
    if(check)
      { 
