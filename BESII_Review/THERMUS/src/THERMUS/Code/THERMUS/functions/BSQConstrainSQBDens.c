...
TTMThermalModelBSQ *gModelBSQConSQBDens;
Double_t gBSQySQBDens[2];

Int_t BSQConstrainSQBDens(TTMThermalModelBSQ *model, Double_t nb)
{    

  gModelBSQConSQBDens = model;
  ...
  gBSQySQBDens[0] = model->GetParameterSet()->GetSDens();
  gBSQySQBDens[1] = nb;
  gBSQySQBDens[2] = model->GetParameterSet()->GetB2Q();

  if(gBSQySQBDens[1] == 0.){
    cout<<"Cannot conserve (nb+nbbar) to zero"<<endl;
    return 1;
  }else{
    broydn(x,3,&check,BSQfuncSQBDens);
    BSQfuncSQBDens(3,x,fbroydn);
    if(check)
      { 
